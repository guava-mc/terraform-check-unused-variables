...
def find_tf_files(_dir):
    try:
        ...
        if len(all_tf_files) < 1:
            logging.info(
                f"Did not find any tf files in {target_dir}\nEnsure running "
        ...
        raise IndexError(
            f'Failed to find required variable file "{args.var_file}" in {target_dir}, but did find other '
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Scan terraform module(s) for unused variables and remove them."
    )
...
if __name__ == "__main__":
    ...
    if args.recursive:
        dirs_to_check = [
            ...
            if len(x[0].split("/.")) < 2 and len(x[0].split("\\.")) < 2
        ]
